...
当前数据库主要保存评分后的结构化打卡日志：

- 已保存：`message_id`、用户 ID、用户名、分数、note、运动类型、时长、卡路里、活动摘要、创建时间。
- 未完整保存：原始对话文本、原始图片 key、`chat_id`、`chat_type`、机器人最终回复全文、排行榜/周报查询记录。

如果后续需要做完整对话分析或问题追踪，建议新增 `message_logs` 表。
